...
F_enc = [[0] * n for i in range(m)]
for i in range(m):
	for j in range(n):
		F_enc[i][j] = csys.Enc(F[i,j], gamma)
print(f"Enc(F)={F_enc}")
...
for k in range(len(t)):
	# state encryption
	for i in range(n):
		x_enc[k][i] = csys.Enc(x_[k,i], gamma)
	# encrypted controller
	for i in range(m):
		u_enc[k][i] = csys.Mult(F_enc[i][0], x_enc[k][0])
		for j in range(1, n):
			u_enc[k][i] = csys.Add(u_enc[k][i], csys.Mult(F_enc[i][j], x_enc[k][j]))
	# input decryption
	for i in range(m):
		u_[k,i] = csys.Dec(u_enc[k][i], gamma ** 2)
	# plant update
...
plt.xlim(0, simulation_time)
plt.savefig("./fig/enc_input.eps", dbi=300, bbox_inches='tight', pad_inches=0.05)
...
plt.xlim(0, simulation_time)
plt.savefig("./fig/enc_state_1.eps", dbi=300, bbox_inches='tight', pad_inches=0.05)
...
plt.xlim(0, simulation_time)
plt.savefig("./fig/enc_state_2.eps", dbi=300, bbox_inches='tight', pad_inches=0.05)
