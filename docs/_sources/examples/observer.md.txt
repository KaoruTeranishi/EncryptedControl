...
This example illustrates encrypted observer-based controls using the homomorphic encryption schemes in ECLib.
...
L, _, _ = dlqr(plant.A.T, plant.C.T, np.eye(n), np.eye(l))
L = L.T

Ac = plant.A - plant.B @ K - L @ plant.C
Bc = L
Cc = -K
Dc = 0
xc0 = [0, 0]

controller = system.Controller(Ac, Bc, Cc, Dc, x0=xc0)
...
- ElGamal
    ...
    `python observer.py elgamal`

- Dynamic-key ElGamal

    `python observer.py dyn_elgamal`

- Paillier

    `python observer.py paillier`

- Regev (LWE)

    `python observer.py regev`

- GSW

    `python observer.py gsw`

- GSW-LWE

    `python observer.py gsw_lwe`
