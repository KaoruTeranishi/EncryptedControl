...
def keygen(bit_length):
    p = pu.get_prime(bit_length)
    q = pu.get_prime(bit_length)
    ...
    k = ru.get_rand(0, params.n)
    while gcd(k, params.n) != 1:
        k = ru.get_rand(0, params.n)
    pk = (k * params.n + 1) % params.n_square

    sk = namedtuple("SecretKey", ["lmd", "mu"])
    sk.lmd = nu.lcm(p - 1, q - 1)
    sk.mu = mu.minv(_L(mu.mpow(pk, sk.lmd, params.n_square), params.n), params.n)
...
def elementwise_add(params, c1, c2):
    ...
    # matrix + vector
        ...
        isinstance(c1[0][0], int)
        and isinstance(c2[0], int)
...
def int_mult(params, m, c):
    ...
    # matrix (plaintext) x vector (ciphertext)
        ...
        isinstance(m[0][0], int) and isinstance(c[0], int) and m.shape[1] == c.shape[0]
    ):
    ...
    # matrix (plaintext) x matrix (ciphertext)
        ...
        isinstance(m[0][0], int)
        and isinstance(c[0][0], int)
        and m.shape[1] == c.shape[0]
    ):
        ...
        for i in range(m.shape[0]):
            for j in range(c.shape[1]):
                for k in range(m.shape[1]):
                    c_[i][j] = _add(
                        params, c_[i][j], _int_mult(params, m[i][k], c[k][j])
                    )
        return c_
    else:
...
def _encrypt(params, pk, m):
    ...
    while gcd(r, params.n) != 1:
        r = ru.get_rand(0, params.n)
    return (
        mu.mpow(pk, m, params.n_square) * mu.mpow(r, params.n, params.n_square)
    ) % params.n_square


def _decrypt(params, sk, c):
    return (_L(mu.mpow(c, sk.lmd, params.n_square), params.n) * sk.mu) % params.n
...
def _int_mult(params, m, c):
    return mu.mpow(c, m, params.n_square)
